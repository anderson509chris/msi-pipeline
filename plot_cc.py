...
def panel(axL, axR, T, letter, d, mt, ppm, halfwin):
    ...
    for ax, halfppm in ((axL, None), (axR, 25)):
        ...
        ax.set_xlim(lo, hi); ax.set_ylim(0, ymax * (1.30 if halfppm is None else 1.12))
...
def main():
    ap = argparse.ArgumentParser(description="Plot Fig S1c: raw profile vs centroid sticks over the common-ROI pixel set (clean variant, no metrics box).")
    add_run_dir_args(ap)
    args = ap.parse_args()
