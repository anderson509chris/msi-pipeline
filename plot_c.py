...
def panel(axL, axR, T, letter, d, mt, ppm, halfwin, show_box=True):
    s = d[T]; m = mt[T]; g = s["grid"]; y = s["prof"]
    tol = T * ppm * 1e-6
    has_profile = not np.isnan(m["ymax"])
    has_centroid = len(s["cmz"]) > 0
    if has_profile:
        ymax = m["ymax"]
    elif has_centroid and s["cit"].max() > 0:
        ymax = float(s["cit"].max())
    else:
        ymax = 1.0
    for ax, halfppm in ((axL, None), (axR, 25)):
        ...
        if halfppm is None and has_profile:
    ...
    if show_box:
        lines = []
        if not np.isnan(m["cmz"]):
            lines.append(f"centroid  {m['cmz']:.4f}  ({m['ppm_cent']:+.2f} ppm)")
        if not np.isnan(m["fwhm"]):
            lines.append(f"FWHM  {m['fwhm'] * 1000:.2f} mDa   R = {m['R']:,.0f}")
        nb_txt = f"{m['nb_ppm']:,.0f} ppm" if not np.isnan(m["nb_ppm"]) else "n/a"
        lines.append(f"peaks in ±{ppm:g} ppm: {m['n_in_window']}   nearest other peak: {nb_txt}")
        if m.get("warning"):
...
def main():
    ap = argparse.ArgumentParser(description="Plot Fig S1b: raw profile vs centroid sticks, over the common-ROI pixel set.")
    add_run_dir_args(ap)
    ...
    d = pickle.load(open(cfg.out("spectra_common.pkl"), "rb")); mt = pickle.load(open(cfg.out("metrics_common.pkl"), "rb"))

    fig = plt.figure(figsize=(10.6, 12.4))
    gs = fig.add_gridspec(len(TARGETS) + 1, 2, height_ratios=[.30] + [1] * len(TARGETS), width_ratios=[1.35, 1],
                          hspace=.52, wspace=.20, left=.075, right=.975, top=.965, bottom=.06)
    ...
    for i, (T, L) in enumerate(zip(TARGETS, "ABCDEFGHIJKLMNOPQRSTUVWXYZ")):
        panel(fig.add_subplot(gs[i + 1, 0]), fig.add_subplot(gs[i + 1, 1]), T, L, d, mt, cfg.ppm, cfg.halfwin, show_box=show_box)
    fig.text(.5, .008, cfg.mode_banner(), ha="center", va="bottom", fontsize=6, color="0.4")
    fig.savefig(cfg.out("Fig_S1b_common_ROI.pdf"))
    fig.savefig(cfg.out("Fig_S1b_common_ROI.png"), dpi=300)
    plt.close(fig)
    ...
    for T, L in zip(TARGETS, "ABCDEFGHIJKLMNOPQRSTUVWXYZ"):
        ...
        panel(f.add_subplot(g2[0, 0]), f.add_subplot(g2[0, 1]), T, L, d, mt, cfg.ppm, cfg.halfwin, show_box=show_box)
        f.text(.5, .01, cfg.mode_banner(), ha="center", va="bottom", fontsize=5.5, color="0.4")
        f.savefig(cfg.out(f"commonROI_mz{T:.4f}.png"), dpi=300); plt.close(f)

    with open(cfg.out("peak_metrics_commonROI.csv"), "w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["assignment", "theoretical_mz", "profile_apex_mz",
                    ...
                    "n_pixels_above_intensity_floor", "modes", "warning"])
        pk = np.load(cfg.out("peak_prof.npy" if cfg.has_profile else "peak_cent.npy"))
        for i, T in enumerate(TARGETS):
            ...
            w.writerow([nm, f"{T:.4f}", f"{m['apex']:.5f}", f"{m['pcen']:.5f}",
                        ...
                        int((pk[i] > cfg.intensity_floor).sum()), "+".join(m.get("modes", cfg.modes)), m.get("warning", "")])
    print("OK")
